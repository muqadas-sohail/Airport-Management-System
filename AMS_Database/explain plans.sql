...
EXPLAIN SELECT SZ.ZoneName, COUNT(SO.EmployeeID) AS NumSecurityOfficers
FROM SECURITY_ZONE SZ
LEFT JOIN SEC_SHIFT SS ON SZ.SecurityZoneID = SS.SecurityZoneID
...
EXPLAIN SELECT FlightID, DepartureAirport, ArrivalAirport
FROM FLIGHT
...
EXPLAIN SELECT FirstName, LastName, WorkEmail
FROM EMPLOYEE
WHERE EmployeeID IN (
    SELECT EmployeeID
...
EXPLAIN SELECT EmployeeID, FirstName, LastName, ScheduleCount
FROM (
...
EXPLAIN SELECT so.EmployeeID, e.FirstName, e.LastName, COUNT(s.ShiftID) AS ShiftCount
...
EXPLAIN SELECT F.FlightID, ROUND(AVG(B.Weight)) AS AvgBaggageWeight
FROM FLIGHT F
...
EXPLAIN SELECT EmployeeID, FirstName, LastName, ShiftCount
FROM (
